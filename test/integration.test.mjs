...
function makeApi() {
  ...
  const srv = http.createServer((req, res) => {
      ...
      return json({
        values: [
          { hash: "dddddddddddd4444", message: "feat: 브랜치 커밋\n\n왜 그랬는지",
            ...
            author: { user: { display_name: "김철수" } }, parents: [{ hash: "p" }] },
        ],
      });
    }
      ...
      return json({
        values: [
          ...
          { hash: "bbbbbbbbbbbb2222", message: "Merge branch 'x'",
            ...
            author: { user: { display_name: "홍길동" } }, parents: [{ hash: "p1" }, { hash: "p2" }] },
        ],
      });
    }
    if (sub === "/pullrequests/7/activity") {
      return json({
        values: [
          { update: { author: { display_name: "김철수" }, date: "2026-09-04T00:00:00Z", state: "OPEN" } },
    ...
    if (sub && sub.startsWith("/commit/")) {
      return json({
        ...
        author: { user: { display_name: "이영희" } }, parents: [{ hash: "p" }],
      });
    }
...
test("bb_file_history 는 해시만 오는 이력을 커밋 조회로 채운다", async () => {
  await withServer({}, async ({ callTool }) => {
    ...
    assert.equal(out.history[0].author, "이영희");
