...
된다 — 저장소를 내려받거나 브랜치를 fetch 하지 않는다.

```
/bb-prs                     열린 PR 목록 (현재 폴더의 저장소를 자동 감지)
/bb-review acme/web-app 64  diff·커밋·기존 코멘트를 읽고 한국어 리뷰 초안
/bb-pr-new                  브랜치 커밋으로 PR 초안 → 확인받고 생성
/bb-doctor                  설정 진단 — 문제마다 실행할 명령까지 준다
/bb-repos                   허용 저장소 확인·추가
```

툴 20개가 목록·diff·파일 전문·커밋 이력·활동 이력을 **나눠서** 준다. 한 번에 다 받지
않는 이유는 실측한 PR 하나의 전체 diff 가 77KB 였기 때문이다 — 리뷰 단계에 맞춰
쪼갠 근거는 §3 툴.

**일부러 없는 것 — PR 승인·머지 툴.** 만들지 않았고 요청받아도 만들지 않는다.
리뷰는 판단 근거를 모으는 일이고, 머지 여부를 정하는 것은 사람의 몫이다.

**기본이 차단이다.** 허용 저장소를 지정하지 않으면 **아무 저장소에도 접근하지 않는다.**
쓰기는 게이트 5개로 쪼개져 각각 켜야 한다 — 리뷰하려고 머지 권한을 열 필요가 없다.
경계의 실체와 **못 막는 것 10가지**는 §7 보안.

PR 본문·코멘트·diff 는 남이 쓴 텍스트다. 응답에 `_untrusted` 표시를 붙이지만
**강제력은 없다** — 그 한계도 §7 에 적었다.
...
`/bb-prs` 는 **현재 폴더의 저장소**를 기본으로 보고 거기서 끝낸다 — 리뷰를 시작하지 않는다.
전체를 보려면 `/bb-prs all`. 폴더가 Bitbucket 클론이 아니면 전체로 폴백하고 그 사실을 밝힌다.
...
스킬·명령은 `repo` 인자가 없으면 **전부** 이걸 먼저 부르고, 어느 저장소를 쓰는지
한 줄로 밝힌 뒤 진행한다. 말없이 쓰지 않는다.

| 명령 | 감지 성공 | 감지 실패 |
|---|---|---|
| `/bb-prs` | 그 저장소의 PR | 전체 목록으로 폴백 (이유를 밝힌다) |
| `/bb-review` | 그 저장소의 PR에서 고르게 | 선택창 |
| `/bb-pr-new` | 그 저장소·브랜치로 초안 | 선택창 |

**예외를 두지 않는다.** 프로젝트 폴더에서 부르면 그 프로젝트가 범위다.
전체를 원하면 `all` 이나 저장소를 명시한다.

### 그래도 물어야 하면 선택창으로

감지에 실패했고 인자도 없으면 `AskUserQuestion` 으로 선택창을 띄운다.
평문으로 "어느 저장소인가요?" 라고 되묻지 않는다 — 사용자가 이름을 기억해
타이핑해야 한다.

**옵션은 2~4개까지다.** allowlist 가 그보다 많으면 순위를 정해 상위 4개를 보여주고
나머지는 "Other" 로 직접 입력받는다. 순위는 **열린 PR이 있는 저장소 우선**,
그다음 최근 갱신순 — 리뷰가 목적이니 당장 할 일이 있는 것이 위로 와야 한다.

```
저장소를 고르세요

1) acme/web-app        열린 PR 1건 · 방금
2) acme/admin-web      09-03 갱신
3) acme/api            09-02 갱신
4) acme/mobile         09-02 갱신
   Other (직접 입력)
```

각 옵션에 **고를 근거**를 붙인다. 이름만 나열하면 선택에 도움이 안 된다.

- allowlist 가 4개 이하면 전부 넣는다
- **1개면 묻지 않는다** — 그것을 쓰고 한 줄로 밝힌다
- `allowed: false` 일 때는 선택창을 띄우지 않는다. 그 저장소를 쓰려던 것이므로
  allowlist 추가가 필요하다는 사실을 알리는 게 맞다

### 결과는 마크다운 표로 낸다

툴은 JSON을 돌려준다 — 기계가 읽는 형식이고, 모델이 정렬·필터하고 인라인 코멘트
줄번호를 다루려면 구조가 필요하다. **사용자에게 보이는 것은 표다.** 명령과 스킬이
변환을 맡는다.
...
| # | 제목 | 작성자 | 갱신 |
|---|---|---|---|
| 64 | fix: 투표 마감 시각 KST 표시 💬4 | 김철수 | 09-07 04:39 |
...
코드블록 안에서 공백으로 폭을 맞추지 않는다. 터미널이 GFM 을 렌더링하므로 실제
마크다운 표가 낫다 — 한글과 이모지의 표시 폭이 문자 수와 달라서, 직접 맞추면
열이 밀린다.

| 규칙 | 이유 |
|---|---|
| 범위를 `## ` 제목에 | 표와 붙어 있어 어느 범위를 봤는지 놓치지 않는다 |
| 열 순서 고정 (저장소·`#`·제목·작성자·갱신) | 매번 다르면 눈이 다시 찾아야 한다 |
| 오늘은 `MM-DD HH:MM`, 그 외 `MM-DD` | 상대시간(`N시간 전`)은 쓰지 않는다 — 현재 시각을 모르면 계산이 곧 추측이다 |
| 제목은 티켓 접두사 **뒤부터** 40자 | 앞에서부터 세면 `PROJ-17206: fix: ` 가 절반을 먹어 내용이 잘린다 |
| 행은 15개까지 | 실측 101건. 상한이 없으면 아무도 읽지 않는 표가 나온다 |
| 넘치면 제목에 `최근 15건 / 전체 N건` | 몇 건을 안 보여줬는지 밝힌다 |
| `\|` 이스케이프 | PR 제목은 외부 입력이다. 표를 깨뜨릴 수 있다 |

**승인 상태(🟢/🔴)는 목록에 넣지 않는다.** `bb_pr_list`·`bb_pr_inbox` 응답에 승인
정보가 없다 — `approved_by` 는 `bb_pr_get` 에만 있다. 목록에 넣으려면 PR마다 호출이
하나씩 더 붙는다. 물어보면 대상 5건 이하만 조회하고 몇 건을 봤는지 밝힌다.

표를 쓰지 않는 곳도 있다.

- **3건 이하** — 머리글이 내용보다 길어진다
- **허용 저장소 목록** — 열이 하나뿐이라 나열이 읽기 좋다
- **빈 결과** — 머리글만 있는 표를 만들지 않는다. 한 줄로 말한다
- **리뷰 본문** — ①무엇 ②재현 ③수정이 들어갈 자리가 없어진다.
  `bb-pr-review` 스킬의 고정 포맷을 그대로 쓴다
...
**게시는 즉시 공개된다.** Bitbucket 의 배치 리뷰(`Start review` → `pending` →
`Finish review`)는 UI 전용이고 API 에 생성 경로가 없다. 응답에 `pending` 필드는
있지만(실측) 쓰기가 안 되고, 읽기도 리뷰 세션 소유자로 제한된다. 두 가지가 따라온다.

- 게시 전 확인과 **게시 후 되읽기**가 필수다 — 되돌릴 수단이 없으므로
- 중복 검사에 사각지대가 있다. **다른 사람이 UI 에서 리뷰 중이면 그 `pending`
  코멘트가 `bb_pr_comments` 에 보이지 않는다** — 같은 줄을 이미 지적했는지 알 수 없다
...
전에 3개를 하드코딩해서 새 테스트 파일이 조용히 실행되지 않았다(0.18.0).

따옴표 글롭(`"test/**/*.test.mjs"`)을 먼저 썼다가 되돌렸다. **Node 가 직접 확장하므로
셸에 의존하지 않는다**고 판단했는데, 그 네이티브 글롭이 Node 22 부터라서 **셸 의존을
없애는 대신 Node 버전 의존을 만들었다.** CI 매트릭스가 18·20 에서
`Could not find 'test/**/*.test.mjs'` 로 잡았다. 셸 글롭은 전 버전에서 돈다.
...
**⑩ ~~과거 커밋에는 사내 이름이 남아 있다~~ → 제거했다 (2026-09-08)**

`test/forbidden.test.mjs` 는 **현재 트리**만 검사한다. 히스토리는 보지 않는다.
0.18.0 이전 커밋에 사내 워크스페이스·저장소 이름·실명·티켓 키가 남아 있었다.

처음에는 재작성하지 않기로 했다가 **공개 홍보를 검토하면서 뒤집었다.** 그 결정은
"트래픽이 없다" 는 전제에서 내린 것이었고, 전제가 바뀌면 결론도 바뀐다.

`git filter-repo --replace-text --replace-message` 로 블롭과 커밋 메시지를 함께
치환했다. 40커밋 전부 유지되고, 첫 변경 커밋 이후의 SHA 가 바뀌었다.
재작성 뒤 히스토리 전체를 다시 훑어 대상 토큰이 **블롭·메시지 양쪽에서 0건**임을
확인했다.

**당시 반대 근거 하나가 틀렸다.** "CHANGELOG 와 개발 기록이 인용한 SHA 10여 개가
죽어 새 불일치를 만든다" 고 적었는데, 실제로 세어 보니 **저장소 문서에는 SHA 참조가
하나도 없었다.** 커밋 메시지가 인용한 18개는 전부 첫 변경 커밋 이전이라 SHA 가
그대로였다. 확인하지 않은 것을 근거로 써서 결정을 한 번 미뤘다 —
[`CLAUDE.md` §2](./CLAUDE.md) 가 말하는 그 실수다.

**여전히 남는 것**: 재작성은 회수가 아니다. GitHub 의 도달 불가 객체, 포크, 캐시,
아카이브에는 닿지 않는다. 이미 클론한 사람은 옛 히스토리를 갖고 있다. 노출이
일어난 사실 자체는 되돌려지지 않았고, 앞으로 새로 보는 사람이 트리와 히스토리
어디서도 사내 이름을 만나지 않게 된 것이 얻은 전부다.
...
- **allowlist 파일은 히스토리에 한 번도 들어가지 않았다** — 전수 조사(2026-09-08)로
  `.config/`·`allowed-repos` 경로가 전 커밋에서 0건임을 확인했다. 0.9.0 에서 기본
  위치를 저장소 밖(`~/.config/bb-mcp/`)으로 옮긴 판단이 여기서 값을 냈다. 저장소 안에
  뒀다면 사내 저장소 이름이 커밋에 남아, 트리에서 지워도 히스토리 재작성이 필요했다
...
- 코멘트 게이트와 범용 쓰기 게이트를 나눴다. 리뷰를 하려고 머지 권한까지
  열어줄 필요가 없다.
- allowlist는 사람이 파일로 관리하고 서버는 읽기만 한다. **기동 시 한 번 읽으므로
  고쳐도 재시작해야 먹는다**(0.8.0). 편집은 한 줄로 쉽고, 반영에 재시작이 필요한 것이
  경계의 실체다 — 하이재킹된 에이전트가 파일을 고쳐도 그 세션에서는 못 쓴다.
...
CHANGELOG 는 **무엇을 틀렸고 어떻게 고쳤는지**로 쓴다. 그중 이 저장소 밖에서도
해당하는 것을 따로 정리해 뒀다.

- [**기본값이 직관과 어긋나면, 틀린 건 직관이 아니다**](./docs/posts/기본값이-직관과-어긋나면.md)
  — 설정이 없으면 전역이 열리는 fail-open 기본값이었다. 사용자가 한 번에 반대로
  추측했다는 것은 취향이 아니라 사양이 틀렸다는 증거다. 기본값을 뒤집으면 **전에는
  도달 불가능하던 상태**가 생기고, 그 경로는 아무도 밟아본 적이 없다.

- [**의존을 없앤 게 아니라 옮겼다**](./docs/posts/의존을-없앤-게-아니라-옮겼다.md)
  — 하드코딩된 테스트 목록을 글롭으로 고치며 "셸에 의존하지 않는다" 고 적었다.
  그 문장은 참이고, 동시에 버그를 만들었다. 네이티브 글롭이 Node 22+ 라 셸 의존을
  **버전 의존으로 옮긴** 것이었고 내 머신(Node 24)에서는 보이지 않았다. CI 첫 실행이
  잡았다.
