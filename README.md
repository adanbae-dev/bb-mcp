...
**⑩ 과거 커밋에는 사내 이름이 남아 있다 (앞으로만 막힌다)**

`test/forbidden.test.mjs` 는 **현재 트리**를 검사한다. 히스토리는 검사하지 않는다.
0.18.0 이전 커밋 11개의 파일 내용과 커밋 메시지 1개에 사내 워크스페이스·저장소
이름·실명·티켓 키가 들어 있다(해시 대조로 확인).

**재작성하지 않기로 결정했다.** 근거는 셋이다.

- 노출은 이미 일어났다. `filter-repo` 로 다시 써도 GitHub 의 도달 불가 객체,
  포크, 캐시, 아카이브에는 닿지 않는다 — 회수가 아니라 정리일 뿐이다
- 영향받은 커밋 이후 **모든 SHA 가 바뀐다.** CHANGELOG 와 개발 기록이 인용한 SHA
  10여 개가 전부 죽어, 정리하려는 작업이 새 불일치를 만든다
- 현재 트리는 이미 깨끗하고 테스트가 그 상태를 지킨다. 새로 읽는 사람이 보는 것은
  트리다

**그래서 남는 위험**: 히스토리를 뒤지면 사내 저장소 이름과 사람 이름을 알 수 있다.
치명적이지는 않지만 없는 척하지 않는다. 이 저장소를 공개로 두는 한 그대로다.

이력까지 지워야 하는 값이 생기면(예: 저장소를 조직 계정으로 옮길 때) 그때
