...
"어느 저장소인가요?" 라고 평문으로 되묻지 않는다 — 사용자가 이름을 기억해
타이핑해야 한다.

**옵션은 2~4개까지다.** allowlist 가 그보다 많으면 순위를 정해 상위 4개만 보여주고,
나머지는 자동으로 붙는 "Other" 로 직접 입력받는다.

순위 기준 — **열린 PR이 있는 저장소를 맨 앞에**, 그다음 최근 갱신순.

```
bb_pr_inbox()      열린 PR이 어디에 있나
bb_repos()         갱신 시각
```

두 결과를 합쳐 정렬한다. 리뷰가 목적이므로 당장 할 일이 있는 저장소가 위로 와야 한다.

각 옵션의 `description` 에 **고를 근거**를 넣는다. 이름만 나열하면 선택에 도움이 안 된다.

```
acme/web-app    열린 PR 1건 · 방금 갱신
acme/api         09-03 갱신
```

- allowlist 가 4개 이하면 전부 옵션으로 넣는다. 순위를 매길 필요가 없다
- allowlist 가 1개면 **묻지 않는다.** 그것을 쓰고 한 줄로 밝힌다
- `allowed: false` 로 감지된 경우에는 선택창을 띄우지 않는다 — 그 저장소를 쓰려던
  것이므로 allowlist 추가가 필요하다는 사실을 알리는 게 맞다
...
**뒤 두 절은 커밋에서 대개 뽑을 수 없다.** 그럴 때 비워 두거나 지우지 말고
`확인 필요` 로 남기고 **사용자에게 묻는다** — 없는 절은 "안 물어본 것" 과
구별되지 않는다.
